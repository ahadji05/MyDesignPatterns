 ...
 */
template<size_t BlockSize>
void ContiguousMemoryPool<BlockSize>::deallocate( void *ptr ) {
    try {
        size_t startIndex = this->_find_allocated_block( ptr );
        size_t numBlocks  = this->_find_number_of_allocated_blocks( ptr );
        for ( size_t i = 0; i < numBlocks; ++i )
            this->_release_block( this->m_blocks[startIndex + i] );
        this->m_allocMap.erase( ptr );
    } catch (...) {
        std::cerr << "Attempted to deallocate invalid pointer" << std::endl;
    }
}

template class MemoryPool<1024>;
template class ContiguousMemoryPool<1024>;
